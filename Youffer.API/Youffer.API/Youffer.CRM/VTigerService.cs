// ---------------------------------------------------------------------------------------------------
// <copyright file="VTiger.cs" company="Youffer">
...
namespace Youffer.CRM
{
    ...
    /// </summary>
    public class VTiger : IVTigerService
    {
        ...
        public VTiger(string apiUserName, string apiAccessKey, string apiServiceUrl)
        {
            ...
            Login(apiUserName, apiAccessKey);
        }
        ...
        private string VTigerExecuteOperation(string operation, string parameters, bool post)
        {
            ...
            try
            {
                dynamic obj = JsonConvert.Import(response);
                if (obj["error"] != null && obj["error"]["code"].ToString() == "INVALID_SESSIONID")
                {
                    Login(apiUserName, apiAccessKey);
                    string toReplace = Regex.Match(parameters, @"\=([^\}]+)\&").Groups[1].Value;
                    parameters = parameters.Replace(toReplace, sessionName);
                    if (post)
                    {
                        response = HttpPost(baseUrl + operation, parameters);
                    }
        ...
        private T VTigerGetJson<T>(string operation, string parameters, bool post)
        {
            ...
            if (!string.IsNullOrWhiteSpace(response) && response.IndexOf('{') >= 0)
            {
        ...
        public VTigerCompanyNotes AddCompanyNotes(string note, string assigned_user_id, string contact_id, string related_to)
        {
            //Todo: For some kind of reason the server returns success, without creating a new element
            VTigerCompanyNotes element = new VTigerCompanyNotes(note, assigned_user_id, contact_id, related_to);
            return Create<VTigerCompanyNotes>(element);
        }

        public VTigerUserReviews AddUserReview(VTigerUserReviews review)
        {
            //Todo: For some kind of reason the server returns success, without creating a new element
            VTigerUserReviews element = review;
            return Create<VTigerUserReviews>(element);
        }
        ...
        public static Type[] VTigerTypeClasses = 
        {
            ...
            typeof(VTigerCurrency), typeof(VTigerDocumentFolder), typeof(VTigerCompanyNotes)
        };
    }
